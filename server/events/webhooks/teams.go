...
// TeamsWebhook sends webhooks to Slack.
...
func NewTeams(wr *regexp.Regexp, br *regexp.Regexp, channel string, client TeamsClient) (*TeamsWebhook, error) {
	if err := client.AuthTest(); err != nil {
		return nil, fmt.Errorf("testing teams authentication: %s. Verify your slack-token is valid", err)
	}
...
func (s *TeamsWebhook) Send(_ logging.SimpleLogging, applyResult ApplyResult) error {
